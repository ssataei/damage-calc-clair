...
//				}
				...
				for (var n = 0; n < 4; n++) {
					...
					if (maxDamage > highestDamage && !attacker.moves[n].name.includes("Explosion")) {
						...
						var koHits = result.kochance(false).text.substring(result.kochance(false).text.indexOf("HKO")-1, result.kochance(false).text.indexOf("HKO"));
						koHits = (koHits === "O" ? 1 : koHits);
						if (mode === "all-vs-one"){
							if(!koHits.includes("guaranteed")){
								koHits += 1;
							}
						}
						data.push(koHits);
